...
export async function submitEvidence(args: { profileId: string; questId: string; notes?: string; imageData?: string; imageUrl?: string }) {
  ...
  if (!r.ok) throw new Error((await r.json().catch(()=>null))?.error || `Submit failed (${r.status})`)
  return r.json()
}
...
export async function getEvidenceStatus(profileId: string) {
  ...
  if (!r.ok) throw new Error((await r.json().catch(()=>null))?.error || `Status failed (${r.status})`)
...
export async function adminList(params: { status?: EvidenceStatus | 'all' } = {}) {
  ...
  if (!r.ok) throw new Error((await r.json().catch(()=>null))?.error || `List failed (${r.status})`)
  return r.json()
}
...
export async function adminDecision(args: { id: string; decision: EvidenceStatus; reward?: number }) {
  ...
  if (!r.ok) throw new Error((await r.json().catch(()=>null))?.error || `Decision failed (${r.status})`)
  return r.json()
}
